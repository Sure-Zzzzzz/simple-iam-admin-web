...
IAM 管理端微前端，承载用户、组织、角色、权限、可信应用和站内信等 IAM 管理页面，以 `/micro/iam/` 为基路径由门户宿主挂载。
...
开发服务器固定使用 `5175`，基路径为 `/micro/iam/`。`/iam` 与 `/oauth2` 代理至 IAM Server；Portal（`5176`）通过该入口挂载 Admin。
